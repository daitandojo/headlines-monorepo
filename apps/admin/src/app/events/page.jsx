...
export default function EventsPage() {
  ...
  const handleOptimisticUpdate = useCallback(
    async (event, updateData) => {
      // DEFINITIVE FIX: If updateData is empty, it's a signal from a child component
      // (like RelationshipManager) to refetch the main list data.
      if (Object.keys(updateData).length === 0) {
        toast.info('Refreshing event list to reflect relationship changes.')
        refetch()
        // The expanded item's details are refetched internally by its own logic.
        return
      }

      // Proceed with standard optimistic update for direct edits.
      setData((currentData) =>
