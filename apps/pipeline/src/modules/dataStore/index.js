...
export async function savePipelineResults(articlesToSave, eventsToSave) {
  logger.info(
    `Committing pipeline results to databases (MongoDB & Pinecone)...`,
  );
  let savedEvents = [];
  ...
  try {
      ...
      for (const event of savedEvents) {
        ...
        pineconeVectors.push({
          ...
          metadata: {
            ...
            key_individuals: (event.key_individuals || [])
              .map((p) => p.name)
              .join(", "),
    ...
    if (articlesToSave && articlesToSave.length > 0) {
      const articleOps = [];
      const eventKeyToIdMap = new Map(
        savedEvents.map((e) => [e.event_key, e._id]),
      );
      const articleIdToEventKeyMap = new Map();

      for (const event of eventsToSave) {
        for (const sourceArticle of event.source_articles) {
          const article = articlesToSave.find(
    ...
    if (pineconeVectors.length > 0) {
      ...
      logger.info(
        `Pinecone commit complete. Upserted ${pineconeVectors.length} vectors.`,
      );
    }
          ...
          metadata: {
            ...
            summary:
              (Array.isArray(opp.whyContact)
                ? opp.whyContact.join(" ")
                : opp.whyContact) || "",
            country: Array.isArray(opp.basedIn)
              ? opp.basedIn.join(", ")
              : opp.basedIn,
...
export async function filterFreshArticles(
  articles,
  isRefreshMode = false,
  ...
  if (forceReprocess) {
    logger.info(
      `📢 REPROCESS MODE: Forcing reprocessing of all ${articles.length} scraped articles.`,
    );
    return articles;
  }
