...
export async function sendBulkEmails(emailQueue) {
  ...
  for (const { user, events, opportunities } of emailQueue) {
    try {
      if (!events || events.length === 0) {
        logger.info(
          `Skipping email for ${user.email} as it contained no valid events.`,
        );
        continue;
      }
      ...
      // Group opportunities by country too
      const opportunitiesByCountry =
        opportunities?.length > 0
          ? groupItemsByCountry(opportunities, "basedIn")
          : {};
      ...
      if (!htmlBody) {
        logger.error(
          `Failed to generate email body for ${user.email}. Skipping.`,
        );
        continue;
      }
      ...
      if (targetLanguage !== "English") {
        logger.info(
          `Translating email for ${user.email} into ${targetLanguage}...`,
        );
        // DEFINITIVE FIX: Use direct await calls instead of
