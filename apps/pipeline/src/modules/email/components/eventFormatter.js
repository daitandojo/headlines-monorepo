...
async function getOpportunitiesForEvent(eventId) {
  try {
    if (!eventId) return [];
    const opportunities = await Opportunity.find({ events: eventId }).lean();
    return opportunities.map((opp) => ({
      ...opp,
      likelyMMDollarWealth:
        opp.lastKnownEventLiquidityMM ??
        opp.profile?.estimatedNetWorthMM ??
        0,
    }));
  } catch (error) {
    logger.error(
...
function formatTransactionDetails(details) {
  ...
  const flowText =
    flow?.nature && flow?.approxAmountUSD != null
      ? `${flow.nature} (~$${flow.approxAmountUSD}M)`
      : flow?.nature
        ? flow.nature
        : "";
...
async function createEventBriefCard(event) {
      ...
      (article) =>
        `<tr>
          <td style="padding: 6px 0;">
            <a href="${article.link}" target="_blank" style="display: inline-block; padding: 8px 16px; background: #2a2a2a; border: 1px solid #444; border-radius: 6px; color: #58a6ff; text-decoration: none; font-size: 13px; margin-right: 8px; margin-bottom: 6px;">
              📰 ${article.newspaper || "Source"}
            </a>
          </td>
        </tr>`,
    )
  ...
  const allText =
    `${synthesized_headline} ${synthesized_summary}`.toLowerCase();
  const hasNegativeContent = negativeKeywords.some((kw) =>
    allText.includes(kw),
  );

  // Don't show negative events
...
export async function formatEventForEmail(event) {
  ...
  try {
    ...
    logger.error(
      `Error creating event card for email: "${event.synthesized_headline}"`,
      {
