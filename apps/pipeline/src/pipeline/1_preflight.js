...
async function sendPipelineFailureAlert(errorMessage, details = {}) {
  ...
  try {
    await sendGenericEmail({
      to: env.ADMIN_EMAIL || 'reconozco@gmail.com',
...
async function validateApiKeys() {
  ...
  // 1. Kimi K2 (Optional - used for web_search background research only)
  // Core pipeline functions (headline assessment, enrichment, synthesis, clustering)
  // all use DeepSeek. Kimi failure is non-critical.
  if (env.KIMI_API_KEY) {
    try {
      ...
      logger.warn(`⚠️ Kimi failed (optional): ${errMsg}`);
    }
  } else {
    results.kimi = { valid: false, error: "Not configured (optional)" };
    logger.warn("⚠️ Kimi not configured (optional)");
  }
