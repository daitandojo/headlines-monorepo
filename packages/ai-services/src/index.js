...
export * from "./enrichment/conduitLLMExtractor.js";
export * from "./enrichment/familyOfficeSearch.js";
export * from "./enrichment/localizedSearch.js";
...
export async function processUploadedArticle(item, userId) {
  ...
  try {
    ...
    const synthesizedResult = await synthesizeEvent(
      [enrichedArticle],
      [],
      "",
      "",
    ...
    if (
      ...
      throw new Error(
        "AI failed to synthesize an event from the provided text.",
      );
    }
    ...
    const opportunitiesToSave = await generateOpportunitiesFromEvent(
      eventToSave,
      [enrichedArticle],
    );
    ...
    if (opportunitiesToSave.length > 0) {
      await Opportunity.insertMany(
        opportunitiesToSave.map((opp) => ({
          ...opp,
          events: [eventToSave._id],
        })),
      );
    }
...
export async function addKnowledge(data) {
  ...
  try {
    ...
    return {
      success: true,
      message: "Knowledge successfully added and embedded.",
    };
  } catch (error) {
