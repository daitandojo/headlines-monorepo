...
export const getInstructionArticle = () => ({
  ...
  verificationProtocol: [
    ...
    ``,
    // --- START OF CORRECTION ---
    `**C. CONFIRMATION vs. SPECULATION TEST (REVISED)**`,
    `• **"Exploring sale", "considering options", "planning IPO", "in early talks"** = HIGH PREDICTIVE VALUE (score 70-85). This is a critical signal for advisors to engage *before* a transaction.`,
    `• **"Agreed to sell", "has acquired", "transaction closed"** = HIGHEST VALUE (score 90-100). This represents an immediate liquidity event.`,
    `• **"Sources say", "rumored to be"** = MEDIUM-HIGH VALUE (score 65-75). Credible rumors are still actionable intelligence.`,
    // --- END OF CORRECTION ---
    ``,
    ...
    ``,
    `**H. PE FUND ACTIVITY SIGNAL**`,
    `• PE fund acquiring companies, creating a platform roll-up, or making a significant add-on acquisition = MEDIUM VALUE (score 40-65), even without naming individual sellers.`,
    `  - Known Nordic PE funds: Axcel, EQT, Altor, Polaris, GRO Capital, IK Partners, Rivean Capital, FSN Capital, Herkules, Verdane, Summa Equity`,
    `  - This reveals fund deployment strategy, portfolio value creation, and future exit potential.`,
    `  - The fund's partners are UHNW individuals who benefit from carried interest on successful exits.`,
    `  - Exception: Pure PR press releases about fund size or fundraising with no deal activity = LOW VALUE (score 10-20)`,
    `  - Distinguish from: large-cap PE buying from another PE (less personal, score 20-35) vs. PE building a new platform from multiple acquisitions (more signal, score 50-65)`,
  ],
  ...
  scoringRubric: [
    ...
    ``,
    `**70-84: SILVER TIER (HIGH-VALUE PREDICTIVE SIGNALS)**`,
    `• **CRITICAL:** A significant private company "planning IPO", "exploring a sale", "hiring advisors for an exit", or "in talks to be acquired". These are top-priority pre-liquidity events.`,
    ...
    `• Leadership succession at significant private company (strong predictive signal)`,
    `• Major family office investment activity with named principals`,
    ``,
    `**50-69: BRONZE TIER**`,
    `• Credibly reported but speculative liquidity events ("sources say", "rumored to be")`,
