...
export const instructionJudge = {
  ...
  guidelines: [
    ...
    '',
    '   **Enriched Context Fields (USE THESE):**',
    '      - `isOnWatchlist`: If true, the entity is already in our watchlist → boost rating by at least one tier',
    '      - `watchlistPriority`: If set, use it: "high" = minimum Excellent, "medium" = minimum Good',
    '      - `transactionReadinessScore`: If score >= 80 (urgent) → minimum Good; if >= 60 (hot) → minimum Acceptable',
    '      - `transactionReadiness`: Values "urgent" or "hot" = strong signal of actionability',
    '      - `transactionScore`: Numeric 0-100. High scores strongly indicate actionability',
    '      - `wealthEstimateMM`: If > $50M estimated → boost rating (confirmed UHNW individual)',
    '      - `dealValueMM`: If > $50M → strongly supports Excellent/Good rating',
    '      - `keyIndividuals`: Names extracted by the pipeline. Named individuals = research hooks, not irrelevant',
    '',
